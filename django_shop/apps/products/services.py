...
def has_user_reviewed_product(user: Any, product: Product) -> bool:
    return Review.objects.filter(user=user, product=product).exists()

def has_user_purchased_product(user: Any, product: Product) -> bool:
    return Order.objects.filter(user=user, items__product=product, status__in=Order.PAID_STATUS_CHOICES).exists()

def check_user_for_review_eligible(user: Any, product: Product) -> bool:
